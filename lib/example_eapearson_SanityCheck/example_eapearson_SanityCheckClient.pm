package example_eapearson_SanityCheck::example_eapearson_SanityCheckClient;
...
example_eapearson_SanityCheck::example_eapearson_SanityCheckClient
...
sub new
{
    ...
    my $self = {
	client => example_eapearson_SanityCheck::example_eapearson_SanityCheckClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is an example_eapearson_SanityCheck.FilterContigsParams
$output is an example_eapearson_SanityCheck.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is an example_eapearson_SanityCheck.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is an example_eapearson_SanityCheck.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "example_eapearson_SanityCheck.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "example_eapearson_SanityCheck.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "example_eapearson_SanityCheck.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for example_eapearson_SanityCheck::example_eapearson_SanityCheckClient\n";
    }
    if ($sMajor == 0) {
        warn "example_eapearson_SanityCheck::example_eapearson_SanityCheckClient version is $svr_version. API subject to change.\n";
    }
}
...
package example_eapearson_SanityCheck::example_eapearson_SanityCheckClient::RpcClient;
